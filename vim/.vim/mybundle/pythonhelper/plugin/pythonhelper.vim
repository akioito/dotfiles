...
def findTag(bufferNumber, changedTick):
    ...
    # handle possible exceptions {{{
        ...
        # spit out the error {{{
        # testIto print "ERROR: %s %s %s:%u" % (ec.__name__, ei, tb.tb_frame.f_code.co_filename, tb.tb_lineno,)
        time.sleep(0.5)
        vim.command("let w:PHStatusLine=\"%s\"" % ('',))
        # }}}
    # }}}
    # }}}
...
function! PHCursorHold()
    ...
    execute 'Py findTag(' . expand("<abuf>") . ', ' . b:changedtick . ')'
endfunction
...
function! PHBufferDelete()
    ...
    execute 'Py deleteTags(' . expand("<abuf>") . ')'
endfunction
