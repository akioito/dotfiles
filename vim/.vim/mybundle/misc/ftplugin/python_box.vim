...
"Check Syntax
noremap <buffer><A-2> :call CheckPythonSyntax()<CR>:cl<CR>
"Execute current file
noremap <A-F5> :make %<CR><CR>:clast<CR>:cl<CR>
"Key to call outline window in nosplit mode:
map <F8> :call StartOutline("nosplit")<RETURN>
...
function! DefineOutlineNavigationMaps()
	...
	map <silent><buffer> o       :call NavigateToOutlineTag(1)<CR>
    "quit outline buffer:
...
nmenu <silent>&PythonBox.Check\ Syntax<Tab>Alt-2 <A-2>
nmenu <silent>&PythonBox.Run\ current\ File<Tab>Alt-F5 <A-F5>
nmenu &PythonBox.-Sep0- :
"this requires having sourced buftoc.vim:
nmenu <silent> &PythonBox.Buffer\ TOC<Tab>F5 :call StartOutline("nosplit")
...
nmenu <silent> &PythonBox.Comment\ Selection<Tab>Cmd-1 :call PythonCommentSelection()<CR>
vmenu <silent> &PythonBox.Comment\ Selection :call PythonCommentSelection()<CR>
nmenu <silent> &PythonBox.Uncomment\ Selection<Tab>Cmd-2 :call PythonUncommentSelection()<CR>
...
"compiler python "commented because error when called from mvim
